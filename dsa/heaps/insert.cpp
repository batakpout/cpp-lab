/*
    -In worst case we have to compare new node with each root, so O(H) in worst case but in CBT
        height is O(Log(N)) so complexity will be O(Log(n))

    -Also as we are going floor(n-1)/2 in each step , next N/4....till we reach ....1 so that is also the reason for O(Log(N)) to base 2
